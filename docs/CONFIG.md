...
|RelayConfig.is_fast|optional bool| Critical blocks go only to fast relays.<br>A block is consider critical if either has true block value > fast_bid_threshold_eth or it contains replaceable orders.|true|
...
|fast_bid_threshold_eth|optional string|Bids above this value will only go to fast relays.| "0"|
## Building algorithms
