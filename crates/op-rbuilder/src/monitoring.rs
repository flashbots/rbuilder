...
use reth_node_api::{Block, FullNodeComponents, NodeTypes};
use reth_optimism_node::node::OpPrimitives;
    ...
    Node: FullNodeComponents<Types: NodeTypes<Primitives = OpPrimitives>>,
{
    ...
    async fn revert(&mut self, chain: &Chain) -> eyre::Result<()> {
...
fn decode_chain_into_builder_txs(
    chain: &Chain,
    builder_signer: Option<Signer>,
) -> Vec<(&SealedBlockWithSenders, &TransactionSigned)> {
    chain
