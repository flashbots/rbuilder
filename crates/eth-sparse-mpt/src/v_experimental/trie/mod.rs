...
impl Trie {
    ...
    pub fn insert_nibble_key(
        ...
        let ins_key_vec = nibbles_key.to_vec();
        let ins_key = ins_key_vec.as_slice();
    ...
    pub fn delete_nibbles_key(
        ...
        let del_key_vec = nibbles_key.to_vec();
        let del_key = del_key_vec.as_slice();
        ...
        if self.nodes.is_empty() {
            ...
            match node {
                ...
                ProofNode::Extension { key, child } => {
                    let nibbles_key = proof_store.keys_guard()[*key].clone();
