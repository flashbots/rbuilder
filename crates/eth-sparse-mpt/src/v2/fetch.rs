...
pub fn check_state_root_in_db(
    ...
    let factory = DatabaseTrieCursorFactory::new(provider.tx_ref());
    let mut cursor = factory
        .account_trie_cursor()
        .map_err(SparseTrieError::other)?;
    let root_node = cursor
        .seek_exact(reth_trie::Nibbles::default())
        .map_err(SparseTrieError::other)?;
    match root_node {
        Some((_, node)) => {
            let Some(node_root_hash) = node.root_hash else {
                return Err(SparseTrieError::Other(eyre::eyre!(
                    "reth db account trie has no hash in root node"
                )));
            };
            if node_root_hash == expected_state_root {
                Ok(())
            } else {
                Err(SparseTrieError::WrongDatabaseTrieError)
            }
        }
        // reth db trie has no root node, this should only be possible on empty db tests
        None => Err(SparseTrieError::Other(eyre::eyre!(
            "reth db account trie has no root node"
        ))),
    }
}
