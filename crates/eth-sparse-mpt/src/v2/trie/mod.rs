...
impl Trie {
                    ...
                    if let Some(child_ptr) = self.branch_node_children[children][n as usize] {
                            ...
                            == 2
                        {
                            ...
                            if orphan_ptr.is_remote() {
                                let mut orphan_path =
                                    Nibbles::from_nibbles_unchecked(&del_key[..(path_walked - 1)]);
        ...
        loop {
            ...
            match node {
                DiffTrieNode::Branch { children } => {
                    ...
                    let n = target_key_vec[path_walked];
                    path_walked += 1;
                ...
                DiffTrieNode::Leaf { key, value } => {
                    if self.keys[key.clone()] == target_key_vec[path_walked..] {
    ...
    fn add_node_from_proof(
        ...
        if self.nodes.is_empty() {
            ...
            match node {
                ...
                ProofNode::Extension { key, child } => {
                    let key_nibbles = proof_store.keys_guard()[*key].to_vec();
                    let key = self.insert_key(&key_nibbles);
        ...
        loop {
            let walked_nibbles = Nibbles::from_nibbles_unchecked(&path_vec[..path_walked]);
            let node = self
                .nodes
                .get(current_node)
                .ok_or_else(|| NodeNotFound::new(&walked_nibbles))?;
            self.hashed_nodes[current_node] = false;
            match node {
                DiffTrieNode::Branch { children } => {
                    let children = *children;

                    let n = path_vec[path_walked] as usize;
                    path_walked += 1;
                    if path_vec[path_walked..].is_empty() {
                    ...
                    if let Some(child_ptr) = self.branch_node_children[children][n] {
                        ...
                        let walked = Nibbles::from_nibbles_unchecked(&path_vec[..path_walked]);
                        return Err(NodeNotFound::new(&walked));
                    }
                }
                DiffTrieNode::Extension { key, next_node } => {
                    let key = key.clone();
                    ...
                    if path_vec[path_walked..].starts_with(&self.keys[key.clone()]) {
                        path_walked += key.len();
                        ...
                        current_node = next_node.as_local().ok_or_else(|| {
                            NodeNotFound(Nibbles::from_nibbles_unchecked(&path_vec[..path_walked]))
                        })?;
                        continue;
                    }
                }
