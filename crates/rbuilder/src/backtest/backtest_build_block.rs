...
use crate::backtest::restore_landed_bundles::{ExecutedBlockTx, restore_landed_bundles, sim_historical_block, SimplifiedOrder};
...
pub async fn run_backtest_build_block<ConfigType: LiveBuilderConfig>() -> eyre::Result<()> {
    ...
    if cli.sim_landed_block {
        ...
        for tx in tx_sim_results {
            println!("{:>74} revert: {:>5} profit: {}", tx.tx.hash(), !tx.receipt.success, format_ether(tx.coinbase_profit));
            executed_orders.push(
                ExecutedBlockTx::new(tx.tx.hash, tx.coinbase_profit, tx.receipt.success)
            )
        }
        ...
        for order in &orders {
            if block_data.built_block_data.as_ref().map(|bd| bd.included_orders.contains(&order.id())).unwrap_or(true) {
                simplified_orders.push(SimplifiedOrder::new_from_order(order));
            }
        }
        let restored_orders = restore_landed_bundles(
            executed_orders,
            simplified_orders,
        );

        for (id, order) in &restored_orders {
            println!("{:>74} total_profit: {}, unique_profit: {}, error: {:?}", id, format_ether(order.total_coinbase_profit), format_ether(order.unique_coinbase_profit), order.error);
        }
        ...
        if let Some(built_block) = &block_data.built_block_data {
            ...
            for included_order in &built_block.included_orders {
                if let Some(order) = restored_orders.get(&included_order) {
                    println!("{:>74} total_profit: {}, unique_profit: {}, error: {:?}", order.order, format_ether(order.total_coinbase_profit), format_ether(order.unique_coinbase_profit), order.error);
