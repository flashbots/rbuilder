...
pub fn sim_historical_block(
    ...
    let ctx = BlockBuildingContext::from_onchain_block(onchain_block, chain_spec, None, HashSet::default(), coinbase, suggested_fee_recipient, None);


    let state_provider = provider_factory
        .history_by_block_hash(ctx.attributes.parent)?;
    let mut partial_block =
        PartialBlock::new(true, None);
    let mut state = BlockState::new(&state_provider);

    partial_block.pre_block_call(&ctx, &mut state).with_context(|| "Failed to pre_block_call")?;
    ...
    for (idx, tx) in txs.into_iter().enumerate() {
        ...
        let result = {
            ...
            fork.commit_tx(&tx, &ctx, cumulative_gas_used, 0, cumulative_blob_gas_used)?.with_context(|| format!("Failed to commit tx: {} {:?}", idx, tx.hash()))?
        };
...
fn find_suggested_fee_recipient(block: &alloy_rpc_types::Block, txs: &[TransactionSignedEcRecoveredWithBlobs]) -> Address {
    let coinbase = block.header.miner;
    let (last_tx_signer, last_tx_to) = if let Some((signer, to)) = txs.last().map(|tx| (tx.tx.signer(), tx.tx.to().unwrap_or_default())) {
        (signer, to)
    } else {
        return coinbase
    };
...
fn extract_onchain_block_txs(onchain_block: &alloy_rpc_types::Block) -> eyre::Result<Vec<TransactionSignedEcRecoveredWithBlobs>> {
