...
impl SimplifiedOrder {
    ...
    pub fn new_from_order(order: &Order) -> Self {
        ...
        match order {
            Order::Tx(tx) => {
                SimplifiedOrder::new(id, vec![OrderChunk::new(vec![(tx.tx_with_blobs.hash(), TxRevertBehavior::AllowedIncluded)], false, 0)])
            }
            Order::Bundle(_) => {
                let txs = order.list_txs().into_iter().map(|(tx, optional)| {
            ...
            Order::ShareBundle(bundle) => {
                SimplifiedOrder::new(id, OrderChunk::chunks_from_inner_share_bundle(&bundle.inner_bundle))
            }
        }
    }
}
...
impl OrderChunk {
    pub fn new(txs: Vec<(B256, TxRevertBehavior)>, optional: bool, kickback_percent: usize) -> Self {
        OrderChunk {
            txs,
            optional,
            kickback_percent,
    ...
    pub fn chunks_from_inner_share_bundle(inner: &ShareBundleInner) -> Vec<Self> {
        ...
        let mut release_chunk = |current_chunk_txs: &mut Vec<(B256, TxRevertBehavior)>, accumulated_chunks: &mut Vec<OrderChunk>, kickback_percent| {
            if !current_chunk_txs.is_empty() {
                accumulated_chunks.push(OrderChunk {
                    txs: current_chunk_txs.drain(..).collect(),
        ...
        for (idx, body) in inner.body.iter().enumerate() {
            ...
            if prev_element_payed_refund != current_element_pays_refund {
                ...
                release_chunk(&mut current_chunk_txs, &mut accumulated_chunks, chunk_refund_percent);
                ...
                ShareBundleBody::Bundle(inner_bundle) => {
                    ...
                    for chunk in &mut inner_chunks {
                        if current_element_pays_refund {
                            chunk.kickback_percent = multiply_inner_refunds(chunk.kickback_percent, chunk_refund_percent);
                        }
                        if inner.can_skip {
                            chunk.optional = true;
                        }
...
impl LandedOrderData {
    pub fn new(order: OrderId, total_coinbase_profit: I256, unique_coinbase_profit: I256, error: Option<OrderIdentificationError>, overlapping_txs: Vec<(OrderId, B256)>) -> Self {
        LandedOrderData {
            order,
...
impl ExecutedBlockData {
    ...
    fn find_tx(&self, hash: B256) -> Option<(usize, &ExecutedBlockTx)> {
        self.txs_by_hash.get(&hash).map(|idx| (*idx, &self.block_txs[*idx]))
    }
...
pub fn restore_landed_bundles(block_txs: Vec<ExecutedBlockTx>, orders: Vec<SimplifiedOrder>) -> HashMap<OrderId, LandedOrderData> {
    ...
    for order in orders {
        match find_landed_bundle_data(&executed_block_data, &order) {
            Ok(data) => {
                for (tx, kickback) in data.landed_txs {
                    txs_to_orders.entry(tx).or_default().push((order.id, kickback));
                }
            }
            Err(e) => {
                result.insert(
    ...
    for (tx, orders) in txs_to_orders {
        ...
        for (order, kickback) in &orders {
            ...
            let profit = if *kickback == 0 {
                ...
                profit.checked_sub(kickback).unwrap_or_default().try_into().unwrap_or_default()
            };
            entry.total_coinbase_profit += profit;
            if orders.len() == 1 {
                entry.unique_coinbase_profit += profit;
            } else {
                entry.overlapping_txs = orders.iter().filter_map(|(o, _)|
                    if *o != order {
                        Some((*o, tx))
                    } else {
                        None
    ...
    for (_, landed_data) in &mut result {
...
fn find_order_chunk(block_data: &ExecutedBlockData, chunk: &OrderChunk) -> Result<FoundChunkData, OrderIdentificationError> {
    let mut txs = Vec::new();
...
fn find_landed_bundle_data(block_data: &ExecutedBlockData, order: &SimplifiedOrder) -> Result<FoundOrderData, OrderIdentificationError> {
...
mod tests {
    ...
    use crate::primitives::{Bundle, MempoolTx, Refund, ShareBundle, ShareBundleTx, TransactionSignedEcRecoveredWithBlobs};
    use super::*;

    fn id(id: u64) -> OrderId {
        OrderId::Tx(h(id))
    }
    ...
    fn tx(id: u64) -> TransactionSignedEcRecoveredWithBlobs {
        TransactionSignedEcRecoveredWithBlobs {
            tx: TransactionSignedEcRecovered::from_signed_transaction(TransactionSigned {
                hash: h(id),
    ...
    fn assert_result(executed_txs: Vec<ExecutedBlockTx>, orders: Vec<SimplifiedOrder>, expected: Vec<LandedOrderData>) {
        let got = restore_landed_bundles(executed_txs, orders);
        assert_eq!(expected.len(), got.len());
        for expected_result in expected {
            let got_result = got.get(&expected_result.order).expect(&format!("Order not found: {:?}", expected_result));
    ...
    fn test_simple_block_identification() {
        let executed_block =
            vec![
                // random mempool tx
                ExecutedBlockTx::new(h(1), i(11), true),
                // bundle 1 with 1 tx
                ExecutedBlockTx::new(h(2), i(12), true),
                // bundle 2 with 2/3 landed txs
                ExecutedBlockTx::new(h(3), i(13), false), // tx can revert
                ExecutedBlockTx::new(h(33), i(14), true),
                // random mempool tx
                ExecutedBlockTx::new(h(4), i(14), true),
                // bundle with simple kickback
                ExecutedBlockTx::new(h(5), i(15), true),
                ExecutedBlockTx::new(h(6), i(16), true), // backrun 1
                ExecutedBlockTx::new(h(7), i(-14), true), // kickback payout
                // last tx in the block
                ExecutedBlockTx::new(h(8), i(-20), true),
            ];

        let orders = vec![
            // bundle 1 with 1 tx
            SimplifiedOrder::new(id(0xb1), vec![
                OrderChunk::new(vec![(h(2), TxRevertBehavior::NotAllowed)], false, 0)
            ]),
            // bundle 2 with 2/3 landed txs
            SimplifiedOrder::new(id(0xb2), vec![
                OrderChunk::new(vec![
                    (h(3), TxRevertBehavior::AllowedIncluded),
                    (h(33), TxRevertBehavior::NotAllowed),
                    (h(333), TxRevertBehavior::AllowedExcluded), // this tx never landed
                ], false, 0)
            ]),
            // bundle with simple kickback
            SimplifiedOrder::new(id(0xb3), vec![
                OrderChunk::new(vec![
                    (h(5), TxRevertBehavior::AllowedIncluded),
                ], false, 0),
                OrderChunk::new(vec![
                    (h(6), TxRevertBehavior::NotAllowed),
                ], false, 90)
            ]),
        ];
        ...
        let results = vec![
            // bundle 1 with 1 tx
            LandedOrderData::new(id(0xb1), i(12), i(12), None, vec![]),
            // bundle 2 with 2/3 landed txs
            LandedOrderData::new(id(0xb2), i(13 + 14), i(13 + 14), None, vec![]),
            // bundle with simple kickback
            LandedOrderData::new(id(0xb3), i(15 + 2), i(15 + 2), None, vec![]),
        ];
    ...
    fn test_merged_sandwich_identification() {
        ...
        let executed_block =
            vec![
                ExecutedBlockTx::new(h(0x11), i(0x11), true),
                ExecutedBlockTx::new(h(0x21), i(0x21), true),
                ExecutedBlockTx::new(h(0xaa), i(0xaa), true),
                ExecutedBlockTx::new(h(0x12), i(0x12), true),
                ExecutedBlockTx::new(h(0x22), i(0x22), true),
            ];

        let orders = vec![
            SimplifiedOrder::new(id(0xb1), vec![
                OrderChunk::new(vec![
                    (h(0x11), TxRevertBehavior::NotAllowed),
                    (h(0xaa), TxRevertBehavior::NotAllowed),
                    (h(0x12), TxRevertBehavior::NotAllowed),
                ], false, 0)
            ]),
            SimplifiedOrder::new(id(0xb2), vec![
                OrderChunk::new(vec![
                    (h(0x21), TxRevertBehavior::NotAllowed),
                    (h(0xaa), TxRevertBehavior::NotAllowed),
                    (h(0x22), TxRevertBehavior::NotAllowed),
                ], false, 0)
            ]),
        ];

        let results = vec![
            LandedOrderData::new(id(0xb1), i(0x11 + 0xaa + 0x12), i(0x11 + 0x12), None, vec![(id(0xb2), h(0xaa))]),
            LandedOrderData::new(id(0xb2), i(0x21 + 0xaa + 0x22), i(0x21 + 0x22), None, vec![(id(0xb1), h(0xaa))]),
        ];
        assert_result(executed_block, orders, results);
    }
    ...
    fn test_merged_backruns_identification() {
        ...
        let executed_block =
            vec![
                ExecutedBlockTx::new(h(0x00), i(12), true),
                ExecutedBlockTx::new(h(0x03), i(2000), true),
                ExecutedBlockTx::new(h(0x02), i(1000), true),
            ];

        let orders = vec![
            SimplifiedOrder::new(id(0xb1), vec![
                OrderChunk::new(vec![
                    (h(0x00), TxRevertBehavior::NotAllowed),
                ], false, 0),
            ]),
            SimplifiedOrder::new(id(0xb2), vec![
                OrderChunk::new(vec![
                ...
                OrderChunk::new(vec![
                    (h(0x02), TxRevertBehavior::NotAllowed),
                ], false, 90),
            ]),
            SimplifiedOrder::new(id(0xb3), vec![
                OrderChunk::new(vec![
                    (h(0x00), TxRevertBehavior::AllowedExcluded),
                ], true, 0), // this is how we merge backruns now
                OrderChunk::new(vec![
                    (h(0x03), TxRevertBehavior::NotAllowed),
                ], false, 80),
            ]),
        ];

        let results = vec![
            LandedOrderData::new(id(0xb1), i(12), i(0), None, vec![(id(0xb2), h(0x00)), (id(0xb3), h(0x00))]),
            LandedOrderData::new(id(0xb2), i(12 + 100), i(100), None, vec![(id(0xb1), h(0x00)), (id(0xb3), h(0x00))]),
            LandedOrderData::new(id(0xb3), i(12 + 400), i(400), None, vec![(id(0xb1), h(0x00)), (id(0xb2), h(0x00))]),
        ];
        assert_result(executed_block, orders, results);
    }
    ...
    fn test_bundle_identification_errors() {
        let executed_block =
            vec![
                ExecutedBlockTx::new(h(0x01), i(11), false),
                ExecutedBlockTx::new(h(0x02), i(12), true),
                ExecutedBlockTx::new(h(0x03), i(12), true),
            ];

        let orders = vec![
            SimplifiedOrder::new(id(0xb1), vec![
                OrderChunk::new(vec![
                    (h(0x01), TxRevertBehavior::NotAllowed),
                ], false, 0),
            ]),
            SimplifiedOrder::new(id(0xb2), vec![
                OrderChunk::new(vec![
            ...
            SimplifiedOrder::new(id(0xb4), vec![
                OrderChunk::new(vec![
                    (h(0x03), TxRevertBehavior::NotAllowed),
                ], true, 0), // this is how we merge backruns now
                OrderChunk::new(vec![
            ...
            SimplifiedOrder::new(id(0xb5), vec![
                OrderChunk::new(vec![
                    (h(0x01), TxRevertBehavior::NotAllowed),
                ], true, 0),
            ]),
        ...
        let results = vec![
            LandedOrderData::new(id(0xb1), i(0), i(0), Some(OrderIdentificationError::TxReverted(h(0x01))), vec![]),
            LandedOrderData::new(id(0xb2), i(0), i(0), Some(OrderIdentificationError::TxNotFound(h(0xAA))), vec![]),
            LandedOrderData::new(id(0xb3), i(0), i(0), Some(OrderIdentificationError::TxIsIncorrectPosition), vec![]),
            LandedOrderData::new(id(0xb4), i(0), i(0), Some(OrderIdentificationError::TxIsIncorrectPosition), vec![]),
            LandedOrderData::new(id(0xb5), i(0), i(0), Some(OrderIdentificationError::NoOrderTxs), vec![]),
        ];
        assert_result(executed_block, orders, results);
    }
    ...
    fn test_simplified_order_conversion_mempool_tx() {
        let order = Order::Tx(MempoolTx { tx_with_blobs: tx(0x01) });
        let expected = SimplifiedOrder::new(OrderId::Tx(h(0x01)), vec![
            OrderChunk::new(vec![
                (h(0x01), TxRevertBehavior::AllowedIncluded),
            ], false, 0),
        ]);
    ...
    fn test_simplified_order_conversion_bundle() {
        let bundle = Order::Bundle(Bundle {
            ...
            reverting_tx_hashes: vec![h(0x02)],
        ...
        let expected = SimplifiedOrder::new(OrderId::Bundle(uuid::uuid!("00000000-0000-0000-0000-ffff00000002")), vec![
            OrderChunk::new(vec![
                (h(0x01), TxRevertBehavior::NotAllowed),
                (h(0x02), TxRevertBehavior::AllowedExcluded),
            ], false, 0),
        ]);
    ...
    fn test_simplified_order_conversion_share_bundle() {
        let bundle = Order::ShareBundle(ShareBundle {
            ...
            block: 0,
            max_block: 0,
            inner_bundle: ShareBundleInner { body: vec![
                    ...
                    ShareBundleBody::Bundle(
                        ShareBundleInner {
                            ...
                            refund: vec![Refund { body_idx: 0, percent: 20 }],
                            refund_config: vec![],
                            can_skip: true,
                            original_order_id: None,
                        }
                ...
                ], refund: vec![Refund { body_idx: 0, percent: 10 }, Refund { body_idx: 1, percent: 20 }, Refund { body_idx: 4, percent: 30 }], refund_config: vec![], can_skip: false, original_order_id: None, },
            signer: None,
            replacement_data: None,
            original_orders: vec![],
            metadata: Default::default(),
        });
        let expected = SimplifiedOrder::new(OrderId::ShareBundle(h(0xb1)), vec![
            OrderChunk::new(vec![
                (h(0x01), TxRevertBehavior::NotAllowed),
                (h(0x02), TxRevertBehavior::AllowedExcluded),
            ], false, 0),
            OrderChunk::new(vec![
                (h(0x03), TxRevertBehavior::AllowedIncluded),
            ], false, 60),
            OrderChunk::new(vec![
                (h(0x11), TxRevertBehavior::NotAllowed),
            ], true, 60),
            ...
            OrderChunk::new(vec![
                (h(0x04), TxRevertBehavior::AllowedIncluded),
            ], false, 0),
        ]);
