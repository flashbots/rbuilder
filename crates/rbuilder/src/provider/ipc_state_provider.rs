...
pub struct IpcStateProvider {
    ...
    // Per block cache. Arc'd so that clones of the provider share the same cache
    // (reth removed the blanket `StateProvider` impl for `Arc<T>`, so we hand out
    // cheap clones instead of `Arc<Self>`).
...
impl StateProvider for IpcStateProvider {
    ...
    /// Lookup by pre-hashed storage key is not possible over the plain-key RPC interface.
