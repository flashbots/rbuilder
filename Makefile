...
lint: ## Run the linters
	...
	cargo clippy -p op-rbuilder2 --features "$(FEATURES),optimism" -- -D warnings

.PHONY: test
test: ## Run the tests for rbuilder and op-rbuilder
	cargo test --verbose --features "$(FEATURES)"
	cargo test -p op-rbuilder --verbose --features "$(FEATURES),optimism"
	cargo test -p op-rbuilder2 --verbose --features "$(FEATURES),optimism"
...
fmt: ## Format the code
	...
	cargo clippy -p op-rbuilder2 --features "$(FEATURES),optimism" --fix --allow-staged
